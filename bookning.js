...
function initBooking() {
  ...
  if (
    !bookingOverlayElement ||
    !bookingHeaderElement ||
    !bookingContentElement ||
    !bookingCloseButton
  ) {
    return;
  }
...
async function handleBookingStep2Next() {
  ...
  const participantsSelect = document.getElementById(
    "booking-participants-modal"
  );
  ...
  try {
    ...
    if (data.status !== "ok") {
      renderBookingStep2(
        "Booking failed. Please check your data and try again."
      );
      bookingCurrentStep = 2;
      return;
    }
